...
\author[2]{Amrithkala M Shetty}

\authormark{SHENOY K \textsc{and} SHETTY}
...
\address[2]{\orgdiv{Department of Computer Applications}, \orgname{Nitte Institute of Professional Education}, \orgaddress{\state{NITTE, Karnataka}, \country{India}}}
...
\jnlcitation{\cname{%
\author{Vrathik Shenoy K.}, \author{Amrithkala M. Shetty}} (\cyear{2026}),
...
The remainder of this paper is organised as follows. Section~\ref{sec2} reviews the relevant literature on inference energy, quantisation, step reduction, and hardware. Section~\ref{sec3} sets out the benchmarking methodology. Section~\ref{sec4} reports the experimental results together with an updated 2024--2026 landscape. Section~\ref{sec5} discusses the findings and their limitations, and Section~\ref{sec6} closes with the main contributions and future directions.
...
Across the three threads, the literature converges on three points: MoE sparsity outperforms quantisation for LLM energy savings \cite{lopez2025moe}, step reduction dominates quantisation for diffusion models \cite{bertazzini2025hidden}, and hardware generational leaps can dwarf both \cite{nvidia2025gb300}. These threads have so far been studied in isolation, which is the gap our benchmarking study addresses.
...
Four overarching findings emerge, forming a hierarchy of efficiency levers from highest to lowest impact:
...
From a sustainability standpoint, the hierarchy summarised in Section~\ref{sec4} translates directly into deployment guidance, with sparse MoE architectures as the first lever to reach for. The scale of the saving is substantial: deploying GLM-5 (5.9\% active / 744B) in place of a dense 70B model for tasks of comparable effective capacity could cut per-query energy by an estimated 10--20$\times$. At one million daily queries, that is an additional 70,000--160,000~Wh saved per day over the GPT-OSS-20B baseline, or roughly 10--23 metric tonnes of CO$_2$ per year at the US average grid carbon intensity. Quantisation, on this view, is best treated as a complementary fine-grained optimisation rather than the primary efficiency strategy.

A second implication concerns reporting practice. Accuracy and latency are routinely published with new model releases; energy per token or per image deserves the same treatment. Initiatives such as ML.ENERGY \cite{mlenergy2025} and MLPerf's power track \cite{mlperf2025inference} are useful steps in this direction, but industry-wide adoption is still incomplete.
...
\begin{biography}{\raisebox{-5pt}{\includegraphics[width=60pt,height=70pt]{vrathik_bio.jpeg}}}{\textbf{Vrathik Shenoy K.} is a student in the Department of Computer Applications at Nitte Institute of Professional Education, Mangalore, India. Research interests include sustainable AI, energy-efficient machine learning, generative model optimisation, and inference systems design. This work addresses the critical challenge of reducing the environmental footprint of large-scale AI inference.}
\end{biography}

\begin{biography}{\raisebox{-5pt}{\includegraphics[width=60pt,height=70pt]{amrithkala_bio.jpeg}}}{\textbf{Amrithkala M. Shetty} received her MSc degree in computer science from Mangalore University, Karnataka, India. She obtained her PhD from the Department of Computer Science at Mangalore University. Her research interests include machine learning, deep learning, sentiment analysis, and recommendation systems. She is an Assistant Professor at the Nitte Institute of Professional Education, NITTE, Karnataka, India.}
\end{biography}
